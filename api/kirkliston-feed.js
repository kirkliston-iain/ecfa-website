...
export default async function handler(req,res){try{
 ...
 // Do not compare with the server clock here. The ECFA database is the authority: every non-played Kirkliston fixture is upcoming.
 const upcoming=own.filter(f=>f.status!=='played').map(f=>({date:f.fixture_date.slice(0,10),time:new Date(f.fixture_date).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',timeZone:'Europe/London'}),opponent:f.home_team_id===kirk.id?f.awayTeam:f.homeTeam,homeAway:f.home_team_id===kirk.id?'Home':'Away',venue:f.venue,competition:f.competition}));
