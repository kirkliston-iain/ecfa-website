...
function withUnscheduledWeeks(fixtures){if(!fixtures.length)return fixtures;const byDate=new Set(fixtures.map(f=>f.date));const first=new Date(fixtures[0].date+'T12:00:00Z');const last=new Date(fixtures[fixtures.length-1].date+'T12:00:00Z');const out=[...fixtures];for(let d=new Date(first);d<=last;d.setUTCDate(d.getUTCDate()+7)){const date=d.toISOString().slice(0,10);if(!byDate.has(date))out.push({date,time:null,opponent:'Fixture not scheduled yet',homeAway:'Home',venue:'Awaiting ECFA fixture details',competition:'',placeholder:true});}return out.sort((a,b)=>a.date.localeCompare(b.date))}
export default async function handler(req,res){try{
 ...
 // The ECFA database is the authority: every non-played Kirkliston fixture is upcoming.
 // Fill Saturday gaps between known fixtures with an explicit TBC card so players do not mistake an unscheduled week for a confirmed free week.
 const upcoming=withUnscheduledWeeks(own.filter(f=>f.status!=='played').map(f=>({date:f.fixture_date.slice(0,10),time:new Date(f.fixture_date).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',timeZone:'Europe/London'}),opponent:f.home_team_id===kirk.id?f.awayTeam:f.homeTeam,homeAway:f.home_team_id===kirk.id?'Home':'Away',venue:f.venue,competition:f.competition,placeholder:false})));
 ...
 res.status(200).json({source:'ECFA',sourceUrl:'https://ecfa-website.vercel.app/',fetchedAt:new Date().toISOString(),fixtures:upcoming,standings:standings(teams.filter(t=>leagueTeamIds.includes(t.id)),leagueFixtures),form,teams:teams.map(t=>({name:t.name,shortName:t.short_name,logoUrl:t.logo_url}))});
